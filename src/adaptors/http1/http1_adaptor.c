...
// dummy for now:
qd_http_listener_base_t *qd_http1_configure_listener(qd_dispatch_t *qd, const qd_http_bridge_config_t *config, qd_entity_t *entity)
{
    return 0;
}

void qd_http1_delete_listener(qd_dispatch_t *qd, qd_http_listener_base_t *base) {}

qd_http_connector_base_t *qd_http1_configure_connector(qd_dispatch_t *qd, const qd_http_bridge_config_t *config, qd_entity_t *entity)
{
    return 0;
}

void qd_http1_delete_connector(qd_dispatch_t *qd, qd_http_connector_base_t *base) {}
...
typedef struct qdr_ref_adaptor_t {
    ...
    qd_timer_t             *startup_timer;   // invoke "on_startup" once
    qd_timer_t             *activate_timer;  // qdr_http1_connection_activate_CT -> invoke qdr_connection_process on (adaptor->conn)
    qd_timer_t             *stream_timer;    // "on_stream": 1 sec after flow, periodic 100ms on_stream
    qdr_connection_t       *conn;            // qdr_connection_opened

    // created on attach of reply-to (call qdr_link_first_attach), QD_INCOMING target="examples" (address1)
    // send "Test Payload" when credit arrives
    qdr_link_t             *out_link_1;

    // created on attach of reply-to (call qdr_link_first_attach), QD_INCOMING target="stream" (address2)
    // send streaming message when credit available, 1 sec stream timer on first credit, sends 10 batches of data every 100 msec
    // reset delivery on received terminal outcome
    qdr_link_t             *out_link_2;

    // created on attach of reply-to (call qdr_link_first_attach), QD_OUTGOING source="stream" (address2)
    // ??? Not used?
    qdr_link_t             *in_link_2;

    // created on_start after ->conn, QD_OUTGOING, source=dynamic
    qdr_link_t             *dynamic_in_link;
    char                   *reply_to;        // dynamic_in_link source terminus, set on outgoing reply-to msg hdr
 ...
 */
void qdr_http1_adaptor_init(qdr_core_t *core, void **adaptor_context)
{
...
void qdr_http1_adaptor_final(void *adaptor_context)
{
 ...
 */
QDR_CORE_ADAPTOR_DECLARE("http1-adaptor", qdr_http1_adaptor_init, qdr_http1_adaptor_final)
#endif
