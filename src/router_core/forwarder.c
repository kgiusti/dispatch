...
{
    return conn->role == QDR_ROLE_INTER_ROUTER || conn->role == QDR_ROLE_EDGE_CONNECTION;
}
                             ...
                             bool             control)
{
    ...
    const bool    is_streaming         = qd_message_is_streaming(msg);
    ...
    if (!addr->local || exclude_inprocess) {
        ...
        while (link_ref) {
            ...
            if (!qdr_forward_edge_echo_CT(in_delivery, out_link)) {
                ...
                if (is_streaming && use_dynamic_streaming_links(out_link->conn)) {
    ...
    if (origin >= 0) {
        ...
        while (qd_bitmask_first_set(conn_set, &conn_bit)) {
            ...
            } else if (is_streaming) {  // inter-router conns support dynamic streaming links
                           ...
                           bool             control)
{
    ...
    if (link_ref) {
        ...
        if (qd_message_is_streaming(msg) && use_dynamic_streaming_links(out_link->conn)) {
            out_link = get_outgoing_streaming_link(core, out_link->conn);
        }
    ...
    if (addr->next_remote >= 0) {
        ...
        if (rnode) {
            ...
            } else if (qd_message_is_streaming(msg)) {
                            ...
                            bool             control)
{
    ...
    if (chosen_link) {
        ...
        if (qd_message_is_streaming(msg) && use_dynamic_streaming_links(chosen_link->conn)) {
