...
static bool AMQP_rx_handler(void* context, qd_link_t *link)
{
    ...
    //
    // At this point the incoming message has yet to be forwarded AND it is NOT
    // a link-routed message.  Therefore this message will be forwarded across
    // one of the shared inter-router trunk links.
    //
    // We need to determine whether or not this message is "streaming" - a
    // large message that has the potential to block other messages sharing the
    // trunk link.  At this point we cannot for sure know the actual length of
    // the incoming message, so we employ the following heuristic to determine
    // if the message is "streaming":
    //
