...
// timer state machine
//
// IDLE: initial state or state immediately after running.  The timer can be
// scheduled or deleted when in this state
//
...
// SCHEDULED, or DELETED.  The address of the thread executing the callback is
// available in callback_thread
//
...
void qd_timer_free(qd_timer_t *timer)
{
    ...
    if (timer_cancel_LH(timer)) {
        // removed from scheduled_timers, drop internal reference
        assert(sys_atomic_get(&timer->ref_count) > 1);  // assert caller holds ref_count
